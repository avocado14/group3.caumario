...
void g1objfirstposition() {// 리턴 해주어야 하나?.. i가 가로j가 세로
	...
	for (int i = 0; i < 10; i++) {
		obj1x[0][i] = i * 128;
		obj1y[0][i] = 320; //-------------
		g1obj1[0][i] = g1createObject("image/game1/10pixel.png", game1b1, obj1x[0][i], obj1y[0][i], true);
	}
	for (int j = 0; j < 6; j++) {
		obj1x[j][0] = 0 - 10;
		obj1y[j][0] = 720 - j * 120;
		g1obj1[j][0] = g1createObject("image/game1/10pixel.png", game1b1, obj1x[j][0], obj1y[j][0], true);
		obj1x[j][9] = 1280;
		obj1y[j][9] = 720 - j * 120;//|       |
		g1obj1[j][9] = g1createObject("image/game1/10pixel.png", game1b1, obj1x[j][9], obj1y[j][9], true);
	}
	for (int i = 0; i < 10; i++) {
		obj1x[5][i] = i * 128;
		obj1y[5][i] = 0 - 10; //-------------
		g1obj1[5][i] = g1createObject("image/game1/10pixel.png", game1b1, obj1x[5][i], obj1y[5][i], true);
	}
}
	...
	for (int i = 0; i < 10; i++) {
		for (int j = 0; j < 6; j++) {
			if (obj1x[j][i] == 0 && obj1y[j][i]) {//pass
				...
				locateObject(g1obj1[j][i], game1b1, obj1x[j][i], obj1y[j][i]);
			}
		}
	}
}
...
void g1mouseCallback(ObjectID object, int x, int y, MouseAction action) {
...
void g1timerCallback(TimerID timer) {
	...
	if (timer == g1c1move) {//캐릭터움직이는거 전용
		...
		locateObject(g1c1, game1b1, g1c1x, g1c1y);
		setTimer(g1c1move, 0.01f);
		startTimer(g1c1move);
	}
}
void g1soundCallback(SoundID sound) {
	if (sound == theme) {

	}

}
void g1keyboardCallback(KeyCode code, KeyState state)
{
	if (code == 84) {			// UP
...
void maingame1() {
	...
	setMouseCallback(g1mouseCallback);
	setTimerCallback(g1timerCallback);
	setKeyboardCallback(g1keyboardCallback);
	setSoundCallback(g1soundCallback);
	
	game1b1 = createScene("game1", "image/game1/배경.png");

	g1c1 = g1createObject("image/game1/c1.png", game1b1, g1c1x, g1c1y, true);	
	
	
	g1objfirstposition();
	
	g1startbutton = g1createObject("image/game1/확인.png", game1b1, 500, 110, true);
	g1restartbutton = g1createObject("image/game1/다시시작.png", game1b1, 600, 500, true);
